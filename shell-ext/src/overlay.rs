//! "Overlay" leaf entry inside the Offspring root flyout.
//!
//! Off by default — shows when the Overlay tool is enabled and at least
//! one file is selected. Multi-select just runs the overlay encode
//! per-file.
...
impl IExplorerCommand_Impl for OverlayCommand_Impl {
    fn GetTitle(&self, _items: Option<&IShellItemArray>) -> Result<PWSTR> {
        Ok(cotaskmem_wstr("Overlay"))
    }
